...
class Bot:
    def __init__(self, tg_token: str, giga_token):
        ...
        self.giga_bot = GigaChat(credentials=giga_token, verify_ssl_certs=False)
        print("Bot initialized")

    def setup_handlers(self):
        @self.tg_bot.message_handler(commands=['help', 'start'])
        def start_bot(message):
            first_mess = f"<b>{message.from_user.first_name} {message.from_user.last_name}</b>, привет!\nЗадай свой вопрос и я поищу для тебя ответ."



            self.tg_bot.send_message(message.chat.id, first_mess, parse_mode='html')
